...
class WebResearcher:
    ...
    def extract_general_summary_and_keywords(self, combined_text, query_text, num_summary_sentences=7, num_keywords=10):
        ...
        try:
            ...
            selected_scored_sentences = [s[0] for s in sentence_scores[:num_summary_sentences * 2]] 
...
class QueryTopicResearcher: 
    ...
    def get_query_data(self, query_text_input):
        ...
        if is_industry:
            ...
            final_data_payload.setdefault("forecast", {"Overall Market Trend": [100, 110, 121]}) 
            ...
            final_data_payload['raw_text_sample'] = combined_web_text[:1500] 
...
\n---\n{text_snippet}\n---"""
    ...
    def compile_full_report(self, topic_name, sections, fetched_sources=None, query_type="industry_analysis"):
        ...
        toc_items = ["Introduction", "Key Information Summary", "Main Keywords/Topics"] 
        ...
        for i, item_name in enumerate(toc_items):
            anchor = item_name.lower().replace(" ", "-").replace("/", "") 
    ...
    def create_keywords_list_image(self, keywords, width=400, height=300):
        ...
        if not keywords or not isinstance(keywords, list) or not keywords[0]:
            ...
            for i, kw in enumerate(keywords[:10]): 
                ...
                if y_pos < 0.1 and x_start == 0.55: break 
        ax.axis('off')
        buf = io.BytesIO()
        plt.savefig(buf, format='png')
        buf.seek(0)
